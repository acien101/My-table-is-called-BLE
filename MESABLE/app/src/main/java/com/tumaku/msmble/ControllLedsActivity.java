...
public class ControllLedsActivity extends Activity {
    ...
    private final static int WSTATE_SEEKBAR=6;
    ...
    protected void nextState(){
        switch(mState) {
            ...
            case(WSTATE_SEEKBAR):
                String lengthString = Integer.toString(seekBarLength);
                mTextSent.setText("");
                if (Constant.DEBUG) Log.i("JMG","State Write State " + lengthString);
                byte tmpArraySeek []= new byte[lengthString.length()];
                for (int i=0; i<lengthString.length();i++) tmpArraySeek[i]=(byte)lengthString.charAt(i);
                mTumakuBLE.write(TumakuBLE.SENSORTAG_KEY_SERVICE,TumakuBLE.SENSORTAG_KEY_DATA, tmpArraySeek);
                break;
    ...
    private class HM10BroadcastReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (intent.getAction().equals(TumakuBLE.WRITE_SUCCESS)) {
                ...
                if (mState==WSTATE_WRITE_KEY || mState == WSTATE_SEEKBAR) {
                    mState=WSTATE_DUMMY;
                    nextState();
                    return;
                }
